...
const DayView = ({day}) => {
  ...
  return (
    <div className="day-container btn-habit">
      <h5 className="text-center">{day.day}</h5>
      <p className="text-center">{date.getDate()}/{date.getMonth()+1}/{date.getFullYear()}</p>
      <i className={day.isDone===true?"fa-solid fa-circle-check circle-icon active btn-habit":"fa-solid fa-circle-check circle-icon btn-habit"} onClick={markToDone}></i>
      <i className={day.isDone===false?"fa-solid fa-circle-xmark circle-icon active btn-habit":"fa-solid fa-circle-xmark circle-icon btn-habit"} onClick={markToUnDone}></i>
      <i className={day.isDone===""?"fa-solid fa-circle-minus circle-icon active btn-habit":"fa-solid fa-circle-minus circle-icon btn-habit"} onClick={markToNone}></i>
    </div>
  );
};
